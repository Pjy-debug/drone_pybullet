...
DEFAULT_COLAB = False
DEFAULT_TIMESTEPS = int(2e7)   # 总训练步数
...
def linear_schedule_with_min(initial_value, min_lr=1e-4):
...
def run(multiagent=DEFAULT_MA, output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO, local=True, timesteps=DEFAULT_TIMESTEPS, resume=None, task=DEFAULT_TASK, ActionType=DEFAULT_ACT):
    print('===== 训练配置 =====')
    print(f'Multi-agent: {multiagent}')
    print(f'GUI: {gui}')
    ...
    target_reward = 40000. if not multiagent else 60000.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task',               default=DEFAULT_TASK,          type=str,
                        help='场景难度: easy / short / medium / global')
    parser.add_argument('--ActionType',         default=DEFAULT_ACT,           type=str,
                        help='控制方法: RPM / PID')
