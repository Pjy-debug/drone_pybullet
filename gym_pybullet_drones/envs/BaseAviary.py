    ...
    def __init__(self,
        ...
        if self.GUI:
            for i in range(self.NUM_DRONES):
                ...
                # 1. 在初始点画一个小的十字线
                p.addUserDebugLine(init_pos - [0.1, 0, 0], init_pos + [0.1, 0, 0], lineWidth=2, physicsClientId=self.CLIENT)
                p.addUserDebugLine(init_pos - [0, 0.1, 0], init_pos + [0, 0.1, 0], lineWidth=2, physicsClientId=self.CLIENT)
                
                # 2. 在初始点上方写字
                p.addUserDebugText(f"Start_{i}", 
                                   init_pos + [0, 0, 0.2], 
                                   textSize=1.5, 
                                   physicsClientId=self.CLIENT)
